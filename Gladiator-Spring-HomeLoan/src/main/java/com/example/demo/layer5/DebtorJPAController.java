...
public class DebtorJPAController {
	...
	@GetMapping
	@ResponseBody
	@RequestMapping(value = "/getdebt/{email}/{password}")
	public List<Debtor> getDebtor(@PathVariable String email, @PathVariable String password) {
	System.out.println("getdebt()...method ");
	try {
	return depRepo.selectDebtorByEmail(email, password);
	} catch (DebtorNotFoundException e) {
	// TODO Auto-generated catch block
	e.printStackTrace();
	} return null;
